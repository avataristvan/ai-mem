...
def test_fires_for_write(tmp_path: Path) -> None:
    captured, query_uc = _run_main(tmp_path, _payload("Write"))
    assert query_uc.execute.called


def test_fires_for_edit(tmp_path: Path) -> None:
    captured, query_uc = _run_main(tmp_path, _payload("Edit"))
    assert query_uc.execute.called


def test_does_not_fire_for_read(tmp_path: Path) -> None:
    captured, query_uc = _run_main(tmp_path, _payload("Read"))
    assert not query_uc.execute.called


def test_does_not_fire_for_bash(tmp_path: Path) -> None:
    captured, query_uc = _run_main(tmp_path, _payload("Bash"))
    assert not query_uc.execute.called


# ---------------------------------------------------------------------------
# 2. No output — hook is fully silent
# ---------------------------------------------------------------------------

def test_no_stdout_output_on_success(tmp_path: Path) -> None:
    captured, _ = _run_main(tmp_path, _payload())
    assert captured == []


def test_no_stdout_output_on_write(tmp_path: Path) -> None:
    captured, _ = _run_main(tmp_path, _payload("Write", "/repo/src/main.py"))
    assert captured == []
...
# ---------------------------------------------------------------------------
# 5. Query exception does not propagate
# ---------------------------------------------------------------------------
