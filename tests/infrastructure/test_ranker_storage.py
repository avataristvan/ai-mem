...
def _example(memory_id: str, target: float | None = None) -> TrainingExample:
...
def test_prune_keeps_last_n(storage: RankerStorage):
    examples = [_example(str(i)) for i in range(10)]
    storage.append_examples("col", examples)
    storage.prune_buffer("col", keep_last=3)
    loaded = storage.load_buffer("col")
    assert len(loaded) == 3
    assert loaded[0].memory_id == "7"
    assert loaded[-1].memory_id == "9"
